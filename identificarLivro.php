...
<body id="page-top" data-bs-spy="scroll" data-bs-target="#mainNav" data-bs-offset="72" style="width: 100%;height: 100%;">
    ...
    <header class="text-center text-white bg-primary masthead" style="background: rgb(232,237,236);padding: 170px 0px 96px;height: 100%;width: 100%;">
        <div class="container" data-aos="zoom-in-down" data-aos-duration="550" style="height: 550px;">
            <h1 style="font-size: 34px;">Olá, <?php echo "{$_SESSION['nome']}"; ?>!</h1>
            <h1 style="font-size: 40px;">Escaneie o QRCode do Livro</h1>
            ...
            <form id="formulario" method="post" action="application/livro_verificar.php" style="background: rgba(255,255,255,0);">
                <input class="form-control" type="text" style="background: rgba(255,255,255,0);border-color: rgba(33,37,41,0);color: rgba(249,249,249,0); opacity: 0;" name="txtIdLivro" id="txtIdLivro" onBlur="foco()" onkeypress="return somenteN(event)" autocomplete="off">
            </form>
        </div>
    </header>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function(){
            <?php
                If (isset($_GET['erro']) && $_GET['erro'] == '1') {
                    ...
                    echo "$('#modal-body').html('<p>Escaneie o QRCode do livro que você retirou, por favor!</p>');";
                    echo "$('#modal-alerta').modal('show');";
                    echo "var fecharModal = setTimeout(function() {
                        $('#modal-alerta').modal('hide');
                        $('#txtIdLivro').focus();
                    ...
                    echo "$('#modal-body').html('<p>Este livro está indisponível!</p>');";
                    echo "$('#modal-alerta').modal('show');";
                    echo "var fecharModal = setTimeout(function() {
                        $('#modal-alerta').modal('hide');
                        $('#txtIdLivro').focus();
        ...
        window.onload = document.getElementById("txtIdLivro").focus();
        function foco() {
            document.getElementById("txtIdLivro").focus();
        }

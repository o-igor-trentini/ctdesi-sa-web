...
<script>
    ...
    if(!empty($idModal)){
        ...
        echo "
        document.getElementById('idModal').value                 = '".(!empty($dados['id_colaborador']) ? $dados['id_colaborador'] : "Sem Registro")."';
        document.getElementById('tituloModal').innerHTML         = '".(!empty($dados['nome']) ? $dados['nome'] : "Sem Registro")."';
        document.getElementById('cpfModal').innerHTML            = '".(!empty($dados['cpf']) ? $dados['cpf'] : "Sem Registro")."';
        ...
        document.getElementById('usuarioModal').innerHTML        = '".(!empty($dados['usuario']) ? $dados['usuario'] : "Sem Registro")."';
        document.getElementById('tipoModal').innerHTML           = '$tipo';";
        

        $conexao = new BancoDeDados;
        $sql = 'SELECT COUNT(*) AS disponivel FROM emprestimos WHERE id_colaborador=? AND situacao=1';
        $param = [$idModal];
        ...
        // TABELA LOCAÇÕES
        $conexao = new BancoDeDados;
        $sql = 'SELECT a.*,b.titulo as titulo FROM emprestimos as a INNER JOIN livros as b on a.id_livro=b.id_livro WHERE a.id_colaborador=? order by a.id_emprestimo DESC'.$qtdRegistros;
        $param = [$idModal];

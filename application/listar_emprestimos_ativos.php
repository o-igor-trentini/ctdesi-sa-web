<?php
    ...
    if(!empty($dados_requisicao['search']['value'])) {
        $query_qnt_emprestimos .= " AND (a.id_emprestimo LIKE ? OR a.data_retirada LIKE ? OR a.hora_retirada LIKE ? OR b.titulo LIKE ? OR c.nome LIKE ? OR d.nome LIKE ?)";
    }
    ...
    if(!empty($dados_requisicao['search']['value'])) {
        ...
        $param = [$valor_pesq, $valor_pesq, $valor_pesq, $valor_pesq, $valor_pesq, $valor_pesq];
    }
    ...
    if(!empty($dados_requisicao['search']['value'])) {
        $query_emprestimos .= " AND (a.id_emprestimo LIKE ? OR a.data_retirada LIKE ? OR a.hora_retirada LIKE ? OR b.titulo LIKE ? OR c.nome LIKE ? OR d.nome LIKE ?)";
    }
    ...
    foreach($result_emprestimos as $row_emprestimos) {
        ...
        "<form method='post' id='formFinalizar' action='' style='padding:0;'>
            ...
            <a class='btn btn-success btn-circle ms-1' role='button' data-bs-toggle='tooltip' data-bss-tooltip='' id='detalhes' style='margin: 2px !important; background: rgb(24,188,156);width: 35px;height: 35px;font-size: 14px;border-color: rgb(24,188,156);' title='Finalizar Empréstimo' onclick='finalizar({$row_emprestimos['id_emprestimo']})'><i class='fas fa-check text-white'></i></a>
        </form>";
